...
impl<'buf,const HL:usize> HttpHeaders<'buf,HL>{
    ...
    pub  fn new(bytes:&'buf[u8])->Result<HttpHeaders<'buf,HL>,CreatingHeadersErrors>{
...
mod test {
    ...
    #[test]
   fn  check_not_complete_request(){
        let bytes = b"Content-Length: 6\r\nAuthorization: Bearer token\r\n";
        let request = crate::HttpHeaders::<16>::new(bytes);
        assert!(request.is_err());
        let request = request.err();
        assert!(request.is_some());
        let request = request.unwrap();
        assert_eq!(request,crate::CreatingHeadersErrors::ReadMore);
    }
    #[test]
    fn check_requests(){
        let bytes = b"Content-Length: 6\r\nAuthorization: Bearer token\r\n\r\n";
        let headers = crate::HttpHeaders::<16>::new(bytes);
        ...
        assert_eq!(authorization.to_str(), "Bearer token");
    }
}
